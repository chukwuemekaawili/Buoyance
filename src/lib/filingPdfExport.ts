/**
 * Filing PDF Export
 * Generates Buoyance filing record PDFs.
 */
...
function getStatusLabel(status: string): string {
  switch (status) {
    ...
    case "submitted": return "LOCALLY PREPARED";
...
export function generateFilingPDF(filing: Filing): Blob {
  ...
  doc.text("Tax Filing Record", margin, 34);
  ...
  if (filing.submitted_at) {
    doc.text(`Prepared: ${format(new Date(filing.submitted_at), "MMMM d, yyyy 'at' h:mm a")}`, margin, yPos);
    yPos += 5;
  }
  ...
  doc.text(
    "This document is generated by Buoyance as a local filing record.",
    margin,
    yPos
  );
  yPos += 5;
  ...
  yPos += 5;
  doc.text(
    "This does not constitute official acceptance by any tax authority.",
    margin,
    yPos
  );
